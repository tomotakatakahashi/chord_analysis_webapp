...
def audio_wave_to_chromagram_seq(audio_wave, sampling_rate):
    ...
    y_harmonic, y_percussive = librosa.effects.hpss(audio_wave)

    chromagram = librosa.feature.chroma_cqt(
        y=y_harmonic, sr=sampling_rate, bins_per_octave=36
    )

    _, beats = librosa.beat.beat_track(y=y_percussive, sr=sampling_rate)
...
def audio_path_to_chord_seq(audio_path):
    ...
    audio_wave, sampling_rate = librosa.load(audio_path)
